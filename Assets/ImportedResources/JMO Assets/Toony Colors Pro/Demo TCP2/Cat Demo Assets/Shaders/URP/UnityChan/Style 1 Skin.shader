...
Shader "Toony Colors Pro 2/Examples URP/Cat Demo/UnityChan/Style 1 Skin"
{
	Properties
	{
		...
		[MainTexture] _BaseMap1 ("Albedo", 2D) = "white" {}
		[TCP2Separator]
	...
	SubShader
	{
		...
		// Shader Properties
		TCP2_TEX2D_WITH_SAMPLER(_BaseMap1);
		...
		Pass
		{
			...
			) : SV_Target
			{
				...
				float4 __albedo = ( TCP2_TEX2D_SAMPLE(_BaseMap1, _BaseMap1, input.pack0.xy).rgba );
		...
		HLSLINCLUDE
			...
			) : SV_TARGET
			{
				...
				// Shader Properties Sampling
				float4 __albedo = ( TCP2_TEX2D_SAMPLE(_BaseMap1, _BaseMap1, input.pack1.xy).rgba );
				float4 __mainColor = ( _BaseColor.rgba );
				float __alpha = ( __albedo.a * __mainColor.a );
...
/* TCP_HASH b78af2f52d4a119649f3d5ac508654f5 */
